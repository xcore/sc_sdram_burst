...
void speed_regression_single_thread(chanend c_server, unsigned cores) {
...
void latency_regression_single_thread(chanend c_server, unsigned cores) {
  unsigned total = 0;
...
void regression(chanend c_server, chanend in_t, chanend out_t, unsigned cores) {
...
void test_2_threads(chanend c_server) {
  //sdram_server
  speed_regression_single_thread(c_server, 2);
  latency_regression_single_thread(c_server, 2);
}

void test_3_threads(chanend c_server) {
  chan c[2];
  par {
    //sdram_server
    regression(c_server, c[0], c[1], 3);
    load_thread(c[1], c[0]);
  }
}
...
void sdram_client(chanend c_server) {
  ...
  test_3_threads(c_server);
  test_2_threads(c_server);
}

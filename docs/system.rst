...
The basic architecture of SDRAM includes memory cells organized into banks of two dimensional arrays of rows and columns. To address a particular memory cell it is necessary to provide the row and bank address and then select the column in which the desired cell is present. Once a row is accessed, SDRAM allows access of multiple columns in the same row without the need of providing the row and bank address repeatedly. This helps to achieve higher speeds in SDRAM.
